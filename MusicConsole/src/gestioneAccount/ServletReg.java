...
*/
		...
		currentSession.setAttribute("nome", name);
			...
			if(model.doRetrieveByKey(name,encryptedPassword)==null) {
				...
				model1.doSave(pro);
				request.getServletContext().getRequestDispatcher("/login.jsp").forward(request, response); //reindiriziamo alla view		
				return;
			}
				else {
					request.setAttribute("presente", true);
					request.getServletContext().getRequestDispatcher("/registrazione.jsp").forward(request, response); //reindiriziamo alla view
					return;	
				}
