			...
			Collection<AccountUtente> au =  model.doRetrieveAll();
			 for(Iterator<AccountUtente> aa = au.iterator();aa.hasNext();) {
				...
				//decriptiamo la password per controllare se è giusta
				 ...
				 byte[] salt1 = new String("123456789").getBytes();
			        int iterationCount = 40000;
			        int keyLength = 128;
					key = Encryption.createSecretKey(pas.toCharArray(), salt, iterationCount, keyLength);
					key1 = Encryption.createSecretKey(el.getPassword().toCharArray(), salt1, iterationCount, keyLength);
					
					encryptedPassword = Encryption.encrypt(pas, key);//criptiamo quella passata
					
					String encryptedPassword1 =  el.getPassword();
					System.out.println(" password criptata inserita: " + encryptedPassword);
					System.out.println(" password criptata recuperata: " + encryptedPassword1);
					
					String decryptedPassword = Encryption.decrypt(encryptedPassword, key);
					System.out.println(" password decriptata inserita: " + decryptedPassword);
					String decryptedPassword1 = Encryption.decrypt(el.getPassword(), key1);
					
				 if((decryptedPassword.equals(decryptedPassword1))&&(el.getNickname().equals(name))) {
			
					 System.out.println(" password decriptata recuperata: " + decryptedPassword1);
						*/
