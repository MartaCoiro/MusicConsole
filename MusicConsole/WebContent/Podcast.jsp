<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1" import="java.util.*,it.unisa.model.*, Class.*"%>
<!DOCTYPE html>
<html lang="it">
<head>
<%
Podcast pod = (Podcast)session.getAttribute("PodcastI");
%>
<meta charset="UTF-8">
<title><%=pod.getNomePodcast()%></title>
...
function passa(){
	...
	$.getJSON("ServletInf?prezzo=<%=pod.getPrezzo()%>&autore=<%=pod.getIdeatore()%>&nome=<%=pod.getNomePodcast()%>&tipo=Streaming",function(result){
		if(result.flag){
...
<h2 id="testo"><%=pod.getNomePodcast()%> - <%=pod.getIdeatore()%></h2>
 <hr>
  <div class="riempimento1">
 <div  class="costo" >
 <h3> Acquista a : <%=pod.getPrezzo()%>&euro;</h3>
 
 <button id="bott" onClick="passa()">Aggiungi al Carrello</button>
 
 </div>

<div class="Rilassamento img-thumbnail"  style="background-image: url(./imgs/<%=pod.getImgPod()%>)"> 
</div>  
</div>
	
	<h5 class="inf"> <hr> Descrizione</h5>	
 
 <p class="descrizione"> <%=pod.getDescrizione()%>
 </p>
